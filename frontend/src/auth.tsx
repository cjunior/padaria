...
type AuthState =
  ...
  | { ready: true; authenticated: true; userName: string; token: string; error: null };
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = {
    state,
    login: () => beginLogin(),
    register: () => beginLogin("register"),
    logout: () => void keycloak().logout({ redirectUri: logoutRedirectUri() }),
    token: () => keycloak().token ?? state.token,
  };
